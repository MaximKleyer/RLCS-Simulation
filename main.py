...
class Player:
    ...
    def play_game(self, team1, team2, overtime=False):
        ...
        for _ in range(num_iterations):
            # Determine if the player takes a shot
            # Calculate scoring chance based on the saves per game of the opposing team
            ...
            if self.team_name == team1.name:
                scoring_chance = self.shooting_percentage - (self.uncertainty * .2)  - ((
                        team2_saves_per_game / 9) * .03)
            elif self.team_name == team2.name:
                scoring_chance = self.shooting_percentage - (self.uncertainty * .2)  - ((
                        team1_saves_per_game / 9) * .03)
            else:
